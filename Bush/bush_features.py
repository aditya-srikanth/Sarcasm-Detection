...
for line in f:
	...
	if len(contents) >=2:
		...
		s_pnellip = getPosNegEllipsis(input,i_pnellip)# handle None case here
		...
		for feature in line:
			...
			if type(feature) == list:
				for feature_element in feature:
					data[line_index,(feature_element[0])-1] = feature_element[1]

		line_index += 1 
		s_line += str(s_quotes)+' '+str(s_hyperbole) +' '+str(s_pnpunct)+' '+str(s_pnellip)+' '+ str(s_punct)+' '+str(s_interj)+' '
		# s_line += '# '+line
		s_line = s_line.strip()
		f_o1.write(str(line)+'\n')
		# print("line break")
