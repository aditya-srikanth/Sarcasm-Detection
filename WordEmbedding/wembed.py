    ...
    for ind, row in df.iterrows():
        ...
        for token in tokens:
            try:
                ...
                wembed_features = np.reshape(wembed_features,(1,wembed_features.shape[0]))
                if char_embeddings == True:
...
for i in [1, 3, 5]:
    feat_df = wembed_features(df, word2vec_model, tokenizer,char_embeddings=True, window=i)
